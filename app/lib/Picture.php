...
class Picture{
	 ...
	 * @return array
	 */
	public function add($names, $tags, $desc, $files, $server, $user,$gallery_id = ''){
		...
		foreach($info as $v){
			...
			if(is_int($tmp)){
				if(!empty($gallery_id)){
					\lib()->load('Gallery');
					$g = new Gallery($gallery_id, $user->getId());
					$g->add_pic($tmp);
				}
				$rt['list'][] = $tmp;
			} else{
	...
	/**
	 * 选择一些新图片
	 * @param int $number 选择数量
	 * @param int $offset 偏移
	 * @return array|bool
	 */
	public function select_new_pic($number = 15, $offset = 0){
		$number = intval($number);
		if($number < 1){
			$number = 1;
		}
	...
	public function parsePic(&$list, $convert_avatar = true){
		...
		for($i = 0, $l = count($list); $i < $l; $i++){
			...
			$list[$i]['pic_link'] = picture_link($list[$i]['pic_id']);
		}
		return $list;
	}
